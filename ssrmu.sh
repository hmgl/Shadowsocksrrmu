...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/bbr.sh; then
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget https://raw.githubusercontent.com/AENeuro/Shadowsocksrrmu/master/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget https://raw.githubusercontent.com/AENeuro/Shadowsocksrrmu/master/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		wget -N "https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_ver}/libsodium-${Libsodiumr_ver}.tar.gz"
		echo -e "${Info} 解压..."
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
